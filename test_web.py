...
async def generate_wordcloud(
    ...
    try:
        ...
        # 화면에 텍스트 배치
        for i, word in enumerate(words[:100]):  # 최대 100개 단어
            freq = freq_dict[word]
            # 빈도수에 비례하는 크기 (최소 8, 최대 24)
            size = 8 + (freq - min_freq) / (max_freq - min_freq) * 16 if max_freq != min_freq else 16
            ...
            plt.text(x, y, word, fontsize=size, color=(r, g, b), 
