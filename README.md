...
- FastAPI + Uvicorn 기반 웹 서버 구조로 Render.com에 최초 배포
- 한문 텍스트 파일(TXT, HTML) 업로드 후 N-gram(2~4자) 빈도 분석
- 슬라이딩 윈도우 방식의 `NgramAnalyzer` 클래스로 한자 추출 및 카운팅
- 공통 N-gram 교집합 필터링, 빈도수 정렬, 원문 하이라이트 기능 포함
- 다운로드: HTML / DOCX / HWP(HTML 위장) 형식 지원
- 다국어 UI: 한국어 · English · 中文 · 日本語
- 워드클라우드 시각화(서버 사이드 렌더링)
